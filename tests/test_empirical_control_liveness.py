"""Empirical R4 contract: control-liveness verdict rules.
...
from benchmarks.empirical.control_liveness import Control, TraceCheck, verify_control
...
def _record(ok: bool = True, dumps: dict | None = None,
    ...
    return {
        "ok": ok,
        "dumps": dumps or {},
        "player_log": f"/tmp/fake-empirical-{arm}.log",  # replaced in tests that check logs
        "_log_text": log_text,
        "runtime_trace": trace or {},
        "output_dir": f"/tmp/fake-empirical-{arm}-out",
        "runtime_trace_path": f"/tmp/fake-empirical-{arm}-trace.json",
    }
...
class _FakePaths(unittest.TestCase):
    ...
    def _verify(self, control, default, set_rec):
        ...
        import benchmarks.empirical.control_liveness as cl

        for rec, arm in ((default, "default"), (set_rec, "set")):
            rec["player_log"] = f"/tmp/fake-empirical-{arm}.log"
            rec["runtime_trace_path"] = f"/tmp/fake-empirical-{arm}-trace.json"
            rec["output_dir"] = f"/tmp/fake-empirical-{arm}-out"
