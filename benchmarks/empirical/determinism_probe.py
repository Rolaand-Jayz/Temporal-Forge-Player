"""Empirical determinism probe.

Schema ``temporal_forge.empirical.determinism.v1``. Repeatedly executes the same
...
- ``nondeterministic`` — differences exceed the envelope (Empirical defect).
...
SCHEMA = "temporal_forge.empirical.determinism.v1"
...
def run_determinism(
    ...
    for index in range(runs):
        record = run_player(
            ...
            run_id=f"EVS-2-det-{index:02d}", frames=frames, warmup=warmup,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run the Empirical determinism probe."
    )
