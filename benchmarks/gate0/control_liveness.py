"""Gate-0 control liveness proofs.

Schema ``temporal_forge.gate0.liveness.v1``. For every experimental control
...
- ``no_op``            — a trace check failed: the control is a no-op or
                         stale configuration (Gate-0 defect);
...
SCHEMA = "temporal_forge.gate0.liveness.v1"
...
def default_registry() -> list[Control]:
    """Controls exercised by Gate-0; extend only with evidence-backed need."""
    return [
        Control(
...
def run_liveness(
    ...
    for control in controls:
        default_record = run_player(
            ...
            run_id=f"FFW-T0-4-{control.name}-default", frames=frames,
        ...
        set_record = run_player(
            ...
            run_id=f"FFW-T0-4-{control.name}-set", frames=frames,
            warmup=warmup,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Run Gate-0 control-liveness paired captures."
    )
